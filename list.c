...
/**
 * frees memory, removes all remaining list items first
 * @param pList
 * @return
 */
 ...
 * @see lst_deleteList()
 *
 * In addition frees the associated data.
 *
 * @param pList
 * @param fdata "destructor" function
 * @return
 */
...
/**
 * gets element after current one
 * @param pList
 */
...
/**
 * gets element before current one
 * @param pList
 */
...
/**
 * inserts new item has first element in the list
 * @param pList
 * @param pData
 * @return
 * * 1 if successful
...
/**
 * adds new element as last element
 * @param pList
 * @param pData
 * @return
 */
...
/**
 * adds new element before current one(left to it)
 * @param pList
 * @param pData
 * @return
 */
...
/**
 * adds new element after current one(right from it)
 * @param pList
 * @param pData
 * @return
 */
...
/**
 * inserts new element in correct order
 * @param pList
 * @param pData
 * @param cmp
 * @return
 ...
 * @param endidx
 */
static void*   binSearch(List_t *pList, void* needle, int (*cmp)(void* a, void*b), long startidx, long endidx)
{
  void *data;
 ...
 * @param endidx
 */
static void* linSearch(List_t *pList, void *needle, int (*cmp)(void* a, void*b), long startidx, long endidx)
{
  long i;
  for(i = startidx; i <= endidx; i++)
  {
    void *data = lst_getIndexed(pList, i);
    if(!cmp(data, needle))
      return data;
  }
  return NULL;
...
/**
 * gets list element with matching data
 * @param pList
 * @param needle data to look for
...
void*   lst_getMatching(List_t *pList, void* needle, int (*cmp)(void* a, void*b))
{
  ...
  return linSearch(pList, needle, cmp, 0, lst_getNodeCount(pList) - 1);
}
